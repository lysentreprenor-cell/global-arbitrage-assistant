...
async function quickScanSymbol(sym: string, cfgIn?: BotConfig): Promise<QuickSignal | null> {
  ...
  try {
    ...
    const minBufS = Math.ceil(spec.min * 1.10 * Math.pow(10, spec.dec)) / Math.pow(10, spec.dec);
...
async function engineTick() {
  ...
  try {
    ...
    // Buy at least min × 1.10 so the post-fee balance still clears the minimum
    // with a wide margin (fees, rounding, price drift between quote and fill).
    const minBuf = Math.ceil(spec.min * 1.10 * Math.pow(10, spec.dec)) / Math.pow(10, spec.dec);
