...
import { violetInk, brand, ai, ink, alpha } from "@/design/palette";
...
export function AppSidebar() {
  ...
  return (
    <aside style={{
      ...
      background: alpha(violetInk.deepest, 0.97),
      borderRight: `1px solid ${alpha(ai.strong, 0.15)}`,
      ...
      <div style={{
        ...
        borderBottom: `1px solid ${alpha(ai.strong, 0.12)}`,
        display: "flex", alignItems: "center", gap: 10,
      }}>
        <div style={{
          width: 34, height: 34, borderRadius: 9, flexShrink: 0,
          background: `linear-gradient(135deg, ${ai.deep}, ${ai.strong})`,
          display: "flex", alignItems: "center", justifyContent: "center",
          boxShadow: `0 4px 14px ${alpha(ai.strong, 0.4)}`,
        }}>
          <TrendingUp size={16} color={ink.white} />
        </div>
        <div>
          <div style={{ color: brand.gold, fontWeight: 900, fontSize: 12, letterSpacing: 0.8 }}>RESELLASSIST</div>
          <div style={{ color: alpha(ink.white, 0.3), fontSize: 9, letterSpacing: 1.2 }}>AI ARBITRAGE</div>
        </div>
      </div>
      ...
      <nav style={{ flex: 1, padding: "10px 8px", display: "flex", flexDirection: "column", gap: 0, overflowY: "auto" }}>
        {SECTIONS.map(section => (
          <div key={section.title} style={{ marginBottom: 4 }}>
            <div style={{ color: alpha(ink.white, 0.2), fontSize: 9, fontWeight: 700, letterSpacing: 1.2, padding: "10px 10px 4px" }}>
              {section.title}
            </div>
                ...
                <Link key={href} href={href}>
                  <a
                    style={{
                      ...
                      background: active ? alpha(ai.strong, 0.18) : "transparent",
                      color: active ? ai.soft : alpha(ink.white, 0.45),
                      fontWeight: active ? 700 : 500, fontSize: 12,
                      textDecoration: "none", transition: "all 0.12s",
                      border: active ? `1px solid ${alpha(ai.strong, 0.28)}` : "1px solid transparent",
                    }}
                    onMouseEnter={e => { if (!active) { const el = e.currentTarget as HTMLElement; el.style.background = alpha(ink.white, 0.05); el.style.color = ink.white; } }}
                    onMouseLeave={e => { if (!active) { const el = e.currentTarget as HTMLElement; el.style.background = "transparent"; el.style.color = alpha(ink.white, 0.45); } }}
                  >
                    <Icon size={14} />
                    {label}
                  </a>
      ...
      <div style={{
        padding: "10px 16px",
        borderTop: `1px solid ${alpha(ai.strong, 0.1)}`,
        display: "flex", alignItems: "center", justifyContent: "space-between",
      }}>
        <span style={{ color: alpha(ink.white, 0.15), fontSize: 10 }}>AI Arbitrage v2.0</span>
        <NotificationBell />
      </div>
    </aside>
  );
