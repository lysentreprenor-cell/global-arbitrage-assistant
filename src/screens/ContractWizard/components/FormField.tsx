...
export function Input({ value, onChangeText, placeholder, keyboardType = 'default', multiline }: InputProps) {
  return (
    <TextInput
  ...
  return (
    <View style={cgStyles.row}>
      {options.map(opt => (
        <TouchableOpacity
          key={opt.value}
          style={[cgStyles.chip, selected === opt.value && cgStyles.chipActive]}
          onPress={() => onSelect(opt.value)}
          activeOpacity={0.75}
        >
          <Text style={[cgStyles.chipText, selected === opt.value && cgStyles.chipTextActive]}>
            {opt.label}
          </Text>
        </TouchableOpacity>
      ))}
...
export function Toggle({ label, value, onToggle, icon }: ToggleProps) {
  return (
    <TouchableOpacity style={tStyles.row} onPress={onToggle} activeOpacity={0.75}>
      <View style={[tStyles.box, value && tStyles.boxActive]}>
        {value && <Text style={tStyles.check}>✓</Text>}
      </View>
...
export function HintBox({ type, message }: HintBoxProps) {
  const colors = {
...
const tStyles = StyleSheet.create({
  ...
  boxActive: { backgroundColor: C.purpleDim, borderColor: C.purple },
  check: { color: C.purpleLight, fontSize: 12, fontWeight: '700' },
  icon: { marginRight: 6, fontSize: 14 },
  label: { color: C.textSec, fontSize: 14 },
