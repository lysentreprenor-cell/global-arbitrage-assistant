...
const RATE_SUGGESTIONS: { label: string; value: number }[] = [
  { label: '80 zł/m²', value: 80 },
  { label: '120 zł/m²', value: 120 },
  { label: '160 zł/m²', value: 160 },
  { label: '200 zł/m²', value: 200 },
  { label: '280 zł/m²', value: 280 },
];

function RoomPricingCard({ room, onUpdate, onRemove }: { room: Room; onUpdate: (r: Room) => void; onRemove: () => void }) {
  ...
  return (
    <View style={rStyles.card}>
      <View style={rStyles.topRow}>
        ...
        {subtotal > 0 && <Text style={rStyles.subtotal}>{formatCurrency(subtotal)}</Text>}
      ...
      <View style={rStyles.fieldsRow}>
        ...
        <View style={rStyles.field}>
          <Text style={rStyles.fieldLabel}>Cena za m² (PLN)</Text>
          <TextInput
            style={rStyles.input}
      ...
      <View style={rStyles.rateSuggestions}>
        {RATE_SUGGESTIONS.map(s => (
          <TouchableOpacity key={s.value} style={[rStyles.rateChip, room.pricePerM2 === s.value && rStyles.rateChipActive]} onPress={() => onUpdate({ ...room, pricePerM2: s.value })} activeOpacity={0.75}>
            <Text style={[rStyles.rateChipText, room.pricePerM2 === s.value && rStyles.rateChipTextActive]}>{s.label}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      {room.area > 0 && room.pricePerM2 > 0 && (
        <View style={rStyles.calcRow}>
          <Text style={rStyles.calcText}>{room.area} m² × {room.pricePerM2} zł/m² =</Text>
          <Text style={rStyles.calcResult}>{formatCurrency(subtotal)}</Text>
        </View>
      )}
    </View>
  );
...
export default function StepRenovationPricing({ data, updateData, totalAmount }: Props) {
  ...
  const addRoom = (name: string) => {
    if (!name.trim()) return;
    const room: Room = { id: Date.now().toString(), name: name.trim(), area: 0, pricePerM2: 0, scope: [] };
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      <View style={styles.header}>
        ...
        <Text style={styles.stepDesc}>Dodaj pomieszczenia i podaj cenę robocizny za m². Sumę obliczamy automatycznie.</Text>
      </View>

      {data.rooms.map(room => (
        <RoomPricingCard key={room.id} room={room} onUpdate={updateRoom} onRemove={() => removeRoom(room.id)} />
      ))}
      ...
      <View style={styles.extrasSection}>
        ...
        <View style={styles.extraRow}>
          <Text style={styles.extraName}>Materiały (PLN)</Text>
          <TextInput
      ...
      <View style={styles.totalCard}>
        {roomsTotal > 0 && (
          <View style={styles.totalBreakRow}>
            ...
            <Text style={styles.totalBreakValue}>{formatCurrency(roomsTotal)}</Text>
          </View>
        )}
        {data.materialsValue > 0 && (
          <View style={styles.totalBreakRow}>
            <Text style={styles.totalBreakLabel}>Materiały</Text>
            <Text style={styles.totalBreakValue}>{formatCurrency(data.materialsValue)}</Text>
          </View>
        )}
        {data.additionalCosts > 0 && (
          <View style={styles.totalBreakRow}>
            <Text style={styles.totalBreakLabel}>Inne koszty</Text>
            <Text style={styles.totalBreakValue}>{formatCurrency(data.additionalCosts)}</Text>
          </View>
        )}
